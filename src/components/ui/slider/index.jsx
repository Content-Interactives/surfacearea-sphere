...
const SliderPrimitive = {
  ...
  Range: React.forwardRef(({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("absolute h-full bg-slate-700", className)}
      {...props}
    />
  )),
  Thumb: React.forwardRef(({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn("block h-5 w-5 rounded-full border-2 border-slate-700 bg-white ring-offset-white transition-colors focus-visible:outline-none", className)}
      {...props}
    />
  ))
};
  ...
  return (
    ...
    >
      <SliderPrimitive.Track className="relative h-2 w-full grow overflow-hidden rounded-full bg-slate-100">
        <SliderPrimitive.Range className="absolute h-full bg-slate-700" style={{ width: percentage }} />
      </SliderPrimitive.Track>
      <SliderPrimitive.Thumb 
        className="absolute block h-5 w-5 rounded-full border-2 border-slate-700 bg-white ring-offset-white transition-colors focus-visible:outline-none" 
